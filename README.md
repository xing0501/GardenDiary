# GardenDiary
GardenDiary|我的花園日記:以照片文字紀錄植物生活
...
本地儲存資料： 您在應用程式中輸入或產生的所有資料，包括：
植物名稱、小名、科屬、種類、購買日與地、備註。
日記記錄的日期、文字記錄、活動（澆水、施肥等）。
植物代表照片和日記照片的檔案名與實際照片檔案。
...
🌿 疑難排解與常見問題 (FAQ)
Q1: 為什麼我的植物卡片會顯示紅色提醒？ A: 這是我們的智慧照護提醒功能。紅色提醒代表：
澆水提醒： 已超過 7 天未記錄澆水活動。
維護提醒： 已超過 14 天未記錄任何照護活動（澆水、施肥、除草等）。
如何消除： 請為該植物新增一筆日記記錄，並勾選對應的照護活動，提醒將會自動消失。

Q2: 應用程式內的照片儲存在哪裡？ A: 您的照片儲存在 App 內部的 App 沙盒中。
優點： 隱私性高，不會被其他 App 存取，會被 iCloud 自動備份。
注意： **刪除 App 將會永久刪除所有儲存的照片和記錄！**請務必使用「下載到相簿」功能備份重要照片。

Q3: 如何將 App 裡的照片分享到社群媒體？ A: 您可以使用「下載到相簿」功能：
進入植物的日記記錄，點擊照片進入全螢幕模式。
點擊右上角的下載按鈕（↓ 圖示）。
照片將儲存到您的系統相簿中。
接著，您可以從「照片」App 中選取照片，分享到任何社群平台。

Q4: 我可以編輯或刪除已新增的植物資料和日記記錄嗎？ A:
植物資料： 您可以隨時進入植物詳情頁面，點擊「編輯」按鈕修改所有資訊，包括名稱、代表照片和購買資訊。
日記記錄： 您可以在日記記錄列表中選擇編輯已存在的記錄。

📧 聯絡支援
如果您的問題無法透過上述解答獲得解決，或者您發現了應用程式的錯誤（Bug）或有功能建議，請隨時透過電子郵件與我們聯繫。
